...
Another important aspect of supporting secret shared input is that it enables \emph{sourced secure computation.} In our setting three non-coluding servers would be established. The inputs can be provided by the servers themselves or possibly by external parties simply by secret sharing the input among the servers. This model has been gaining traction in industry laterly. Mozilla recently deploy a service to collect telemetry data about Firefox\cite{...} using two non-colluding servers running the Prio protocol\cite{...}. Other examples include privacy preserving machine learning frameworks which support secret shared inputs such as Facebook's Crypten built for PyTorch\cite{...} and Cape Privacy's FTEncrypted built for TensorFlow\cite{...}. 

In all of these examples the ability to perform complex joins on the secret shared inputs and outputs is extremely valuable. For example, most privacy preserving machine learning publications\cite{...,...} assume that the data being trained on has already been joined together. However, without a framework like ours it is unclear how this would be accomplished while preserving privacy. The generality of our approach allows us to solve these problems and many others, e.g. the two applications presented in \sectionref{sec:app}.
