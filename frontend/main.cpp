...
void ComPsi_Intersect(u32 rows, u32 cols = 0, bool sum = false)
{
	...
	auto routine = [&](int i) {
		...
		if (sum)
		{
			...
			Sh3::sbMatrix AA(C.rows(), 64), BB(C.rows(), 64), CC(C.rows(), 64);
			task = eval.asyncEvaluate(task, cir, { &AA, &BB }, { &CC });

			Sh3Encryptor enc;
			if (i == 0)
			{
				Sh3::i64Matrix m(C.rows(), 1);
...
void ComPsi_cardinality(u32 rows)
{
	...
	auto routine = [&](int i) {
		...
		aby3::Sh3::i64Matrix keys = srvs[i].computeKeys(AB, reveals);
...
void ComPsi_threat(u32 rows, u32 setCount)
{
	...
	auto routine = [&](int i) {
		...
		Sh3::sbMatrix AA(C.rows(), 32), BB(C.rows(), 32), CC(C.rows(), 32);
		task = eval.asyncEvaluate(task, cir, { &AA, &BB }, { &CC });
		task = eval.asyncEvaluate(task, cir, { &AA, &BB }, { &CC });
		task.get();
